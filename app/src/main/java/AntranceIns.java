...
public class AntranceIns extends NanoHTTPD  {
    /** AntranceIns端口, 注意要完全kill应用, 防止端口冲突, 暂时不支持多个antranceIns同时运行. */
...
//        }
    ...
    /**
     * 有些应用会自己捕获崩溃, 导致我们的自动捕获失效, 需要我们自己修改app源码, 在崩溃处进行拦截上传.
     * 这个接口可以在app中反射调用, 方便用户制作一个合法的json.
     * 由于参数和getLogJson()有一定差异, 代码不好复用, 就分开写了, 不过二者的格式是完全一致的.
     * 更新: 尽可能不要用这个接口, 更好的做法是关闭app开发者编写的崩溃自动捕捉!
     * @param crashStack 用户自己的崩溃栈信息
     * @param stackTraceOrigin 原始崩溃栈字符串, 一般情况下是crashStack拼接起来
     * @return json格式的日志
     */
...
//         由于AntranceIns是插桩插到app中的, 尽量不要使用外部依赖
    ...
    public Response serve(IHTTPSession session) {
        ...
        if (Method.GET.equals(session.getMethod())) {
            if (uri.equals("/stmtlog")) {
                Log.i("antranceIns", "get /stmtlog");
                String stmtLog = "";
                try {
                    stmtLog = getLogJson(true, null);
                } catch (Exception e) {
                ...
                Log.i("antranceIns", "get /seteventid");
                Map<String, String> parms = session.getParms();
                if (parms.containsKey("id")) {
                    String idStr = parms.get("id");
                    if (idStr != null) {
                        int id = 0;
                        try {
    ...
    static {
        ...
        try {
            antranceIns.start();
            Log.i("antranceIns", "antrance ins start on " + myPort);
        } catch (IOException e) {
            Log.e("antranceIns", "antrance ins start error");
            e.printStackTrace();
        }
